...
for response in tweepy.Paginator(client.search_all_tweets, 
                                query = '((アレジオン) OR (アレグラ) OR (クラリチン)) -is:retweet -is:nullcast has:geo place_country:JP',
                                ...
                                max_results=500):
    time.sleep(1)
    medication_tweets.append(response)

result = []

# Loop through each response object
for response in medication_tweets:
...
# Output to CSV
tweets_location.to_csv('./medication_tweets/medication_tweets_with_location.csv', index = False)